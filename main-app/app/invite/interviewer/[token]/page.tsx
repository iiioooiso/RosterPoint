...
import { acceptInvitation } from "@/app/actions/teams"
...
export default async function InvitePage({ 
  ...
  if (!user) {
    ...
    const returnUrl = `/invite/interviewer/${token}`
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/40">
        <Card className="w-full max-w-md text-center">
          <CardHeader>
            ...
            <CardDescription>
              You've been invited to join the <strong>{invite.department_name}</strong> department as an interviewer.
            </CardDescription>
          </CardHeader>
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/40">
      <Card className="w-full max-w-md text-center">
        <CardHeader>
          ...
          <CardDescription>
            You're signed in as <strong>{user.email}</strong>. Accept the invitation to join the <strong>{invite.department_name}</strong> department.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {urlError && (
          ...
          <form action={async () => {
            ...
            if (res?.error) {
              ...
              redirect(`/invite/interviewer/${token}?error=${encodeURIComponent(res.error)}`)
            }
